...
const FSHADER_SOURCE =
    `precision mediump float; // Precision qualifier
     uniform sampler2D uSampler0; // 取样器，用来接收纹理图像
     uniform sampler2D uSampler1; // 取样器，用来接收纹理图像
     varying vec2 vTexCoord; // 纹理坐标：顶点之间片元的纹理坐标会在光栅化的过程中内插出来
     void main() {
       vec4 color0 = texture2D(uSampler0, vTexCoord);
       vec4 color1 = texture2D(uSampler1, vTexCoord); // 片元着色器从纹理图像上获取纹素的颜色
...
export default {
  ...
  methods: {
    ...
    initTextures () {
      const gl = this.gl
      const texture0 = gl.createTexture() // 创建纹理对象以存储纹理图像
      const texture1 = gl.createTexture() // 创建纹理对象以存储纹理图像
      // 获取USampler的存储位置
      const uSampler0 = gl.getUniformLocation(gl.program, 'uSampler0')
      const uSampler1 = gl.getUniformLocation(gl.program, 'uSampler1')
      const image0 = new Image()
      image0.onload = () => {
        this.loadTexture(texture0, uSampler0, image0, 0)
      }
      image0.src = require('@/assets/sky.jpg')

      const image1 = new Image()
      image1.onload = () => {
        this.loadTexture(texture1, uSampler1, image1, 1)
      }
